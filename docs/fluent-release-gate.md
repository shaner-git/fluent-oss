# Open-Source Runtime Release Gate

This is the public open-source runtime release stack.
...
Open-source runtime expectations:

- Fluent exposes the shared MCP contract
...
- Codex and Claude bundles scaffold against the same local endpoint shape, and OpenClaw scaffolds the same Fluent server as a native `mcp.servers.fluent` entry
- the bundled `openclaw-plugin/fluent` helper remains visibly distinct from the standalone `fluent-openclaw` package line
- Docker image and container smoke checks remain green
- snapshot backup and restore stay format-compatible across supported open-source backends
