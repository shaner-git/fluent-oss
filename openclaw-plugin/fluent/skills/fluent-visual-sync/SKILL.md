...
This skill is the implementation reference for visual-sync patterns in Fluent.

OpenClaw does not ship a documented Fluent visual-sync surface by default in this package. Follow the host routing matrix in [docs/fluent-host-surface-routing-matrix.md](../../../../docs/fluent-host-surface-routing-matrix.md):

- OpenClaw should fall back to canonical data plus text by default
- only use this skill if an operator has explicitly provided an OpenClaw visual equivalent compatible with `visualize:show_widget`
- ChatGPT / MCP Apps-style hosts and Claude-side hosts each have different default render paths and should not be inferred from OpenClaw

## When to use

Use this skill when:

- The user asks for the grocery checklist itself, such as "What's on my grocery list?", "What do I still To buy?", or "Show me this week's grocery list".
