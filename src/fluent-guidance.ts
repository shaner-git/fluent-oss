...
export const FLUENT_GUIDANCE_DOCUMENTS: Record<FluentGuidanceUri, FluentGuidanceDocument> = {
  ...
  'fluent://guidance/style-purchase-analysis': {
    ...
    rules: [
      ...
      'A product page URL is not enough for reliable visual grounding; if preparation returns hostResponseMode=request_candidate_image, do not give a final buy/wait/skip answer yet. Use page extraction only when that tool is exposed in the current host; otherwise ask the user for item details plus a direct product image or uploaded photo.',
      ...
      'Use style_get_purchase_vision_packet after preparation and any needed image handoff to retrieve model-visible candidate and comparator images plus compact closet comparison context.',
      'Inspect the returned images with host vision, make the stylist call as the host agent, then call style_submit_purchase_visual_observations with concrete color, silhouette, material or texture, distinctive-detail observations, and stylist_judgment.',
      ...
      'In ChatGPT/App SDK hosts, after style_submit_purchase_visual_observations returns renderReady=true, call style_show_purchase_analysis_widget with the same stylist_judgment as the final presentation step before or alongside the final buy/wait/skip explanation.',
      'Use style_render_purchase_analysis with the same stylist_judgment for structured non-widget presentation data or text-first clients; do not use it as the default ChatGPT/App SDK finish.',
      ...
      'Never use lane language in user-facing purchase prose, including casual idioms such as opens a lane, tee lane, loafer lane, or same lane. Say wardrobe job, closet gap, part of the wardrobe, slot, or role instead.',
    ...
    defaultFlow: [
      ...
      'For ChatGPT/App SDK hosts, call style_show_purchase_analysis_widget with the same stylist_judgment when renderReady=true, then give the short stylist explanation around the widget.',
    ],
    preferredTools: [
